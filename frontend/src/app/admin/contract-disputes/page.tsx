...
export default function AdminContractDisputesPage() {
  return (
    <AdminLayout title="Contract disputes">
      <Inner />
    </AdminLayout>
  );
}
...
function Inner() {
  ...
  const [rows, setRows] = useState<ContractDisputeRow[] | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [loading, setLoading] = useState(true);

  const [resolve, setResolve] = useState<ContractDisputeRow | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/admin/contract-disputes${showResolved ? '?resolved=true' : ''}`);
      const data = await res.json();
      if (!res.ok) throw new Error(data?.error ?? 'Failed to load');
      setRows(data as ContractDisputeRow[]);
    } catch (e) {
  ...
  function openResolve(d: ContractDisputeRow) {
    setResolve(d);
  ...
  async function submitResolve() {
    ...
    if (resolution === 'split') {
      ...
      if (refundAmount >= resolve.amount) {
        toast.push(
          `Refund amount must be less than the contract total (${formatPrice(resolve.amount, { currency: resolve.currency })}).`,
          { tone: 'warning' },
        );
        return;
      }
    ...
    setBusy(true);
    try {
      const res = await fetch(`/api/admin/contract-orders/${resolve.contractOrderId}/resolve-dispute`, {
        method: 'POST',
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <PageHeader
        ...
        subtitle="Disputes on signed-contract payments. Resolving frees the escrowed funds."
      />
      ...
      {loading ? (
        ...
        <ErrorState title="Could not load contract disputes" body={error} onRetry={load} />
      ) : (rows ?? []).length === 0 ? (
        <EmptyState
          icon="checkCircle"
          title={showResolved ? 'No resolved contract disputes yet' : 'No open contract disputes'}
          body={showResolved ? 'Once you resolve a contract dispute it will show up here.' : 'When a contract dispute is raised it shows up here.'}
        />
      ) : (
                ...
                <tr key={d.id}>
                  <td>
                    <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--fg-1)' }}>
                      {d.contractTitle ?? 'Untitled contract'}
                    </div>
                    <code style={{ fontSize: 11, color: 'var(--fg-4)' }}>{d.contractOrderId.slice(-8)}</code>
                  </td>
                  ...
                  <td style={{ textAlign: 'right', fontSize: 13, fontWeight: 600, color: 'var(--fg-1)' }}>
      ...
      <Dialog
        ...
        subtitle={
          resolve
            ? `${resolve.contractTitle ?? 'Untitled contract'} · ${formatPrice(resolve.amount, { currency: resolve.currency })}`
            : ''
        }
        maxWidth={560}
        footer={
          <>
            ...
            <Button
              tone="client"
              loading={busy}
              disabled={resolution === 'split' && (refundAmount <= 0 || (resolve != null && refundAmount >= resolve.amount))}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          {resolution === 'split' && resolve && (
            <FormField
              ...
              required
              hint={`In dollars. Must be less than the contract total (${formatPrice(resolve.amount, { currency: resolve.currency })}). The remainder transfers to the freelancer.`}
              error={
                refundAmount <= 0
                  ? 'Enter an amount greater than $0.'
                  : refundAmount >= resolve.amount
            ...
            >
              <PriceInput
                ...
                error={refundAmount <= 0 || refundAmount >= resolve.amount}
              />
            </FormField>
          )}
        </div>
