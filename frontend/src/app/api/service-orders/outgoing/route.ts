...
/** GET /api/service-orders/outgoing — list orders where I'm the client. */
export async function GET(): Promise<NextResponse> {
  ...
  const rows = await db
    ...
    .innerJoin(users, eq(users.id, serviceOrders.freelancerId))
    .where(eq(serviceOrders.clientId, session.user.id))
